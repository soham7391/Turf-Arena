...
const LightPillar = ({
  ...
  pillarHeight = 0.4,
  noiseIntensity = 0.5,
  mixBlendMode = 'normal', // Changed from 'screen' so it shows on white!
  pillarRotation = 0,
  quality = 'high'
}) => {
  ...
  if (!webGLSupported) {
    return (
      <div 
        ...
        style={{ mixBlendMode, width: '100%', height: '100%', position: 'absolute', top: 0, left: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0, 0, 0, 0.1)', color: '#888', fontSize: '14px' }}
      >
        WebGL not supported
      </div>
    );
  ...
  return (
    <div 
      ...
      style={{ mixBlendMode, width: '100%', height: '100%', position: 'absolute', top: 0, left: 0 }} 
    />
  );
};
